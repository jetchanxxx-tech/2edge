...
export default {
	async fetch(request, env, ctx) {
		...
		// === 2Edge Multi-User Patch ===
		// Keep single-user UUID derivation for admin login backward compatibility
...
const userID = (envUUID && uuidRegex.test(envUUID)) ? envUUID.toLowerCase() : [userIDMD5.slice(0, 8), userIDMD5.slice(8, 12), '4' + userIDMD5.slice(13, 16), '8' + userIDMD5.slice(17, 20), userIDMD5.slice(20)].join('-');
		...
		// Multi-user support: background refresh user cache
		ctx.waitUntil(refreshUserCache(env));
		// Current authenticated user (set by protocol parsers or subscription lookup)
		// === End 2Edge Patch ===
...
function 解析木马请求(buffer, passwordPlainText) {
	...
	// === 2Edge Multi-User Patch: sha224Password replaced with multi-user check ===
	// (see multiUser.js for the full implementation)
	// === 2Edge Multi-User Patch ===
	...
	// === End 2Edge Patch === // kept for admin auth fallback
